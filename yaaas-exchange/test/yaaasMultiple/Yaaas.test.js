...
const YaaasExchange = artifacts.require("YaaasExchange.sol");
...
contract("YaaasExchange", accounts => {
	let testERC721;
	let testERC20;
	let yaaasExchange;
    let ownerShare = 1;
	...
	beforeEach(async () => {
        ...
        yaaasExchange = await YaaasExchange.new();
	...
	describe("Create new Offer/Auction", () => {
        ...
        it("Expect offer successfuly created", async () => {
            ...
            await testERC721.setApprovalForAll(yaaasExchange.address, true, {from: accounts[1]});
			const res = await yaaasExchange.addOffer(
                    accounts[1],
                    testERC721.address,
                    assetId,
                    testERC20.address,
            ...
            const offer = await  yaaasExchange.offers(testERC721.address, assetId);
        ...
        it("Set Offer price", async () => {
            ...
            await yaaasExchange.setOfferPrice(testERC721.address, assetId, 200, {from: accounts[1]})    
            const offer = await  yaaasExchange.offers(testERC721.address, assetId);
        ...
        it("Set ExpiresAt", async () => {
            ...
            await yaaasExchange.setExpiresAt(testERC721.address, assetId, 1620481368, {from: accounts[1]})    
            const offer = await  yaaasExchange.offers(testERC721.address, assetId);
    ...
    describe("Create/Cancel bid", () => {
        ...
        it("Expect throw erc20 not approved", async () => {
            ...
            await  yaaasExchange.offers(testERC721.address, assetId);
            await expectThrow(    
             yaaasExchange.safePlaceBid(
                testERC721.address,
                assetId,
                testERC20.address,
                100,
        ...
        it("Expect success bid created", async () => {
            await testERC721.setApprovalForAll(yaaasExchange.address, true, {from: accounts[1]});
			let res = await yaaasExchange.addOffer(
                    accounts[1],
                    testERC721.address,
                    assetId,
                    testERC20.address,
            ...
            const offer = await  yaaasExchange.offers(testERC721.address, assetId);
            await testERC20.approve(yaaasExchange.address, price, {from: accounts[2]});
            res = await yaaasExchange.safePlaceBid(
                testERC721.address,
                assetId,
                testERC20.address,
                price,
            ...
            const bid = await yaaasExchange.bidforAuctions(testERC721.address, assetId, accounts[2]);    
        ...
        it("Cancel bid", async () => {
            ...
            const bidC = await yaaasExchange.cancelBid(testERC721.address, assetId,accounts[2], {from: accounts[2]})  
            const bid = await yaaasExchange.bidforAuctions(testERC721.address, assetId, accounts[2]) 
        ...
        it("Accept bid", async () => {
            ...
            await  yaaasExchange.offers(testERC721.address, assetId);
            await testERC20.approve(yaaasExchange.address, price, {from: accounts[2]});
            await testERC20.approve(yaaasExchange.address, price, {from: accounts[3]});
            res = await yaaasExchange.safePlaceBid(
                testERC721.address,
                assetId,
                testERC20.address,
            ...
            await yaaasExchange.acceptBid( testERC721.address, assetId, accounts[2], {from: accounts[1]})  
    ...
    describe("Direct Buy/Sell", () => {
        ...
        it("Create offer successfuly", async () => {
            await testERC721.setApprovalForAll(yaaasExchange.address, true, {from: accounts[2]});
			const res = await yaaasExchange.addOffer(
                    accounts[2],
                    testERC721.address,
                    assetId,
        ...
        it("Buy offer", async () => {
            ...
            await yaaasExchange.buyOffer(testERC721.address, assetId,{from: accounts[3], value:newPrice})
        ...
        it("set owner share", async () => {
			...
			const res = await yaaasExchange.setOwnerShare(
                    1,1,  { from: accounts[0] }
            )
            assert.equal((await yaaasExchange.shares(1)),1)      
        })
