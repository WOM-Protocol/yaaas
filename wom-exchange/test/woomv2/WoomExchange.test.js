...
contract("WoomExchange", accounts => {
    ...
    describe("Direct Buy/Sell", () => {
        ...
        it("Buy offer", async () => {
            ...
            console.log(web3.utils.fromWei(count2ldBalance)+' '+web3.utils.fromWei(count3ldBalance))
            await testERC721.setApprovalForAll(woomExchange.address, true, {from: accounts[2]});
			const res = await woomExchange.addOffer(
                    accounts[2],
                    testERC721.address,
            ...
            await woomExchange.buyOffer(testERC721.address, assetId,{from: accounts[3], value:price})
        ...
        it("set owner share", async () => {
            ...
            assert.equal((await woomExchange.ownerShare()),2)      
        })
